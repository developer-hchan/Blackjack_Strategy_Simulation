...
class Card:
    def __init__(self, number: int, suit: str) -> None:
        ...
        if number < 2 or number > 11:
            raise ValueError("A card with this int value cannot be generated")
        else:
...
class Hand:
    ...
    def texture(self) -> str:
        self.total
        if any(card.number == 11 for card in self.hand):
            return 'soft'
        else:
            return 'hard'
...
def add(hand: list[Card]) -> int:
    ...
    # NOTE: this list comprehension creates a shallow copy of the filtered Card objects, so any adjustments made to the shallow copy change the original object reference 
