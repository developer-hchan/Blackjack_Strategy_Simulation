...
class CalculateExpectedValue(unittest.TestCase):
    def test_calculate_expected_value_within_range(self):
        """
        Making sure the calculated expected value is within the minimum and maximum
        possible values.
        """
        ...
        for k,v in test_dictionary.items():
            ...
            # NOTE: by calling global_data_dictionary directly, it shouldn't maintain its value for the other tests
        ...
        # Output is a dictionary where the key is (17, "hard", 5, "stand"), and the value is the expected value (float)
        output = calculate_expected_value(sim_case=(17, "hard", 5, "stand"), toml_settings=test_settings)

        # Because the calculated expected value takes the game.value / number of sims
